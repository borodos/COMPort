...
//unsigned char* bufrd = new unsigned char[BUFSIZE];
...
static DWORD WINAPI WriteThread(LPVOID)
{
	...
	while (1)
	{
		...
		WriteFile(COMport, charArray, data.size() + 1, &temp, &overlappedwr);
...
static DWORD WINAPI ReadThread(LPVOID)
{
	...
	while (1)
	{
		...
		{
			if (GetOverlappedResult(COMport, &overlapped, &bytesRidden, true)) // было GetOverlappedResult(COMport, &overlapped, &temp, true))
			{
				if ((mask & EV_RXCHAR) != 0)
				{
					ClearCommError(COMport, &bytesRidden, &comstat); // было ClearCommError(COMport, &temp, &comstat)
					btr = comstat.cbInQue;
					if (btr)
					{

						ReadFile(COMport, bufrd, btr, &bytesRidden, &overlapped); // было ReadFile(COMport, bufrd, btr, &temp, &overlapped)
						ReadPrinting();
...
void ReadPrinting(void)
{
	...
	WriteFile(handle, bufrd, bytesRidden, &bytesWritten, 0); // было WriteFile(handle, bufrd, sizeof(bufrd), &bytesWritten, 0)
	memset(bufrd, 0, BUFSIZE);
}

void COMOpen()
{
	string portname = "COM4"; // - Номер порта!!
	DCB dcb;
	...
	else
	{
		...
		handle = CreateFileA("test.txt", GENERIC_WRITE | GENERIC_READ, 0, NULL, OPEN_ALWAYS | TRUNCATE_EXISTING, 0, NULL);
